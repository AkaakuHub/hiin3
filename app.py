...
def extract_landmarks(image, landmarker):
    try:
        ...
        st.write(
            f"デバッグ: 画像形状={image_rgb.shape}, データ型={image_rgb.dtype}, 最大値={image_rgb.max()}, 最小値={image_rgb.min()}"
        )
...
def manual_annotation_mode(
    ...
    if uploaded_base and uploaded_comp1 and uploaded_comp2:
        ...
        if min_points >= 3 and len(set(points_counts)) == 1:
            ...
            st.info(
                f"💡 **ヒント**: 各画像に最低3点ずつ配置してください。現在: {min_points}点"
            )
        ...
        st.info(
            "📷 3つの画像をすべてアップロードしてから手動アノテーションを開始してください。"
        )
...
def draw_manual_points(image, points):
    ...
    for i, point in enumerate(points):
        ...
        # ポイントを描画
        cv2.circle(
            plotted_image, (x, y), point_size + 2, (255, 255, 255), -1
        )  # 白い背景
    ...
    if points:
        ...
        cv2.putText(
            plotted_image, text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1
        )
